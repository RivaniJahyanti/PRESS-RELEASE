...
from utils import tampilkan_header, display_pendapatan_infographic, get_data
from datetime import datetime

# Konfigurasi Halaman
st.set_page_config(
    layout="wide",
    page_title="Kinerja Pendapatan APBN",
    page_icon="💰"
)
...
# Pesan di Sidebar
with st.sidebar:
    st.info("Anda sedang melihat laman **kinerja pendapatan APBN**.", icon="💰")
...
st.markdown("<h1 class='main-title'>KINERJA PENDAPATAN APBN</h1>", unsafe_allow_html=True)
st.markdown("<h3 class='sub-header'>Lingkup KPPN Lhokseumawe</h3>", unsafe_allow_html=True)

# Periode/tanggal
REPORT_YEAR = 2026
st.markdown(f"<p class='period'>Periode: Januari - Februari {REPORT_YEAR}</p>", unsafe_allow_html=True)
...
st.caption("""
**Catatan:**
- **Sumber:** OMSPAN 28 Februari 2026
- Data yang disajikan adalah per tanggal 28 Februari 2026
""")
